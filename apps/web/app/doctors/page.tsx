    ...
    <div className={`min-h-screen bg-[#FAFAFC] text-[#1A1A2E] font-sans ${outfit.className}`}>
      ...
      <main className="p-6 pb-24">
        ...
        <div className="flex gap-3 overflow-x-auto scrollbar-hide mb-8 pb-2">
          {["All", "Psychiatrist", "Psychologist", "Therapist", "Coach"].map((cat) => (
            <button key={cat} className={`px-5 py-2 rounded-full whitespace-nowrap text-sm font-bold transition-colors ${
              cat === "All" ? 'bg-[#1A1A2E] text-white shadow-md' : 'bg-white text-[#8E8E9F] border border-slate-100 hover:border-[#FF8C8C]'
            }`}>
              {cat}
            </button>
          ))}
