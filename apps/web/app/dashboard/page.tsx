...
import { 
  ...
  ChevronLeft, ChevronDown, SkipBack, SkipForward, Pause, ListMusic, Volume2
} from "lucide-react";
...
export default function Dashboard() {
  ...
  const [currentView, setCurrentView] = useState<"main" | "playlist" | "player">("main");
      ...
      <div className="lg:grid lg:grid-cols-12 lg:gap-12">
        ...
        <div className="lg:col-span-7 flex flex-col">
          ...
          <div className="grid grid-cols-2 gap-4 lg:grid-cols-2 lg:gap-6">
            <div onClick={() => { setSelectedCourse("Basics"); setCurrentView("playlist"); }} className="bg-[#FF8C8C] rounded-[2rem] p-5 text-white flex flex-col justify-between aspect-square group cursor-pointer hover:shadow-lg transition-shadow">
            ...
            <div onClick={() => { setSelectedCourse("Happiness"); setCurrentView("playlist"); }} className="bg-[#FFF3B0] rounded-[2rem] p-5 text-[#1A1A2E] flex flex-col justify-between aspect-square group cursor-pointer hover:shadow-lg transition-shadow">
              <div className="self-center w-20 h-20 relative flex items-center justify-center mb-2">
  ...
  const renderPlaylist = () => (
    <div className="animate-in fade-in slide-in-from-right-8 duration-300 mb-24 lg:mb-8">
      ...
      <div className="flex items-center gap-4 mb-8 lg:mb-12">
        <button onClick={() => setCurrentView("main")} className="w-10 h-10 lg:w-12 lg:h-12 bg-white rounded-full flex items-center justify-center shadow-sm text-[#1A1A2E] hover:bg-slate-50 transition-colors">
          <ChevronLeft className="w-6 h-6 lg:w-5 lg:h-5" />
        </button>
        <div>
          <h1 className="text-3xl lg:text-4xl font-bold text-[#1A1A2E]">{selectedCourse || "Course"}</h1>
          <p className="text-[#8E8E9F] text-sm lg:text-base font-medium">17 sessions • 3h 15m</p>
        </div>
      </div>
        ...
        <div className="space-y-4">
          {[
            ...
            <div 
              key={session.id} 
              onClick={() => {
                setSelectedSession({ title: session.title, duration: session.duration });
  ...
  const renderPlayer = () => (
    <div className="animate-in fade-in zoom-in-95 duration-500 fixed inset-0 z-[100] bg-[#FAFAFC] flex flex-col">
      ...
      <div className="px-6 py-6 flex justify-between items-center lg:px-12 lg:py-8">
        <button onClick={() => setCurrentView("playlist")} className="w-12 h-12 bg-white rounded-full flex items-center justify-center shadow-sm text-[#1A1A2E] hover:bg-slate-50 transition-colors">
          <ChevronDown className="w-6 h-6" />
        </button>
        <div className="text-center">
          <p className="text-[#8E8E9F] text-xs font-bold uppercase tracking-widest mb-1">Playing from</p>
          <p className="text-[#1A1A2E] font-semibold">{selectedCourse || "Course"}</p>
        </div>
      ...
      <div className="flex-1 flex flex-col justify-center items-center px-8 max-w-lg mx-auto w-full">
        ...
        <div className="text-center mb-8 w-full">
          <h2 className="text-3xl font-bold text-[#1A1A2E] mb-2">{selectedSession?.title || "Session Title"}</h2>
          <p className="text-[#8E8E9F] text-lg font-medium">Vishal • {selectedCourse}</p>
        </div>
        ...
        <div className="w-full mb-10">
          ...
          <div className="flex justify-between text-[#8E8E9F] text-xs font-bold">
            <span>2:45</span>
            <span>{selectedSession?.duration || "10:00"}</span>
          </div>
        </div>
        ...
        <div className="lg:max-w-5xl lg:mx-auto w-full relative">
          {currentView === "main" && (
            <>
              ...
              {["Sleep", "Exercise"].includes(activeTab) && (
